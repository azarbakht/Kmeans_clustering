...
%     randCenterIndices = randi([1 numberOfTrainingData], 1, K);
    ...
    clusterCenters = data3(randCenterIndices,:);
    
    data3Label = zeros(numberOfTrainingData, 1);
    ...
    while changeInLabelsFlag == 1,
        ...
        for i = 1:numberOfTrainingData,
            for j = 1:K,
                data1DistanceFromClusterCenter(i,j) = EuclideanDistance(data3(i,:),clusterCenters(j,:));
            end
            % check to see if any data point's label is updated
            [~, newLabel] = min(data1DistanceFromClusterCenter(i,:));
            if  data3Label(i) ~= newLabel,
                changeInLabelsFlag = 1;
            end
            % update label maybe?
            [~, data3Label(i)] = min(data1DistanceFromClusterCenter(i,:));
        ...
        for i = 1:K,
            ...
            Mu(i,:) = mean(data3(find(data3Label == i),:));
            
            
            minDiff = EuclideanDistance(data3(1,:), Mu(i,:));
            
            for j = 2:numberOfTrainingData
                if minDiff > EuclideanDistance(data3(j,:), Mu(i,:)),
                    minDiff = EuclideanDistance(data3(j,:), Mu(i,:));
...
%             clusterCenters(i,:) = data1(closestDataPointToMuIndex,:);
    ...
    for i = 1:K,
        ...
        clusterI = data3(find(data3Label == i),:);
